...
function FormControl() {
    ...
    async function fetchFileAsText(url) {
        ...
        // await pro získání Response objektu
        if (!response.ok) {
            throw new Error(`HTTP chyba! Status: ${response.status}`);
        }
        // await pro získání obsahu jako text
    ...
    const ProcessFileContent = async (fileContent, inputFileName, typeSrc) => {
        try {
            ...
            if (detection) {
                ...
                // potřeba upravit ... pokud se dostane špatný soubor a nepůjde naparsovat, nesmí se uložit do DB.
                // Uložení souboru do IndexedDB --> Je v DB ... ale lokální. Není na servru !!!!!!!!!
    ...
    const handleForFile = async (e) => {
        //  přes (e) dostaneš e.target?.files[0]
        setLoading(true);
        ...
        try {
            ...
            fileContent = await new Promise((resolve, reject) => {
                ...
                // Budou binární soubory ???? pak přes buffer !!!! readAsArrayBuffer(file)
